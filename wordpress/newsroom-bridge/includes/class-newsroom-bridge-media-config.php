...
/**
 * Validated, read-only view of deployment-provided media security configuration.
 */
final class Newsroom_Bridge_Media_Config {
	...
	private function __construct() {
		$this->user_id         = self::read_user_id();
		$this->hmac_state      = self::read_boolean_constant( 'NEWSROOM_BRIDGE_MEDIA_HMAC_ENABLED' );
		$this->lockdown_state  = self::read_boolean_constant( 'NEWSROOM_BRIDGE_MEDIA_SERVICE_LOCKDOWN_ENABLED' );
		$this->logging_enabled = true === self::read_boolean_constant( 'NEWSROOM_BRIDGE_SECURITY_LOGGING_ENABLED' );
		$this->max_bytes       = self::read_max_bytes();
		$this->key_ring        = array();
		$this->key_ring_valid  = false;

		if ( defined( 'NEWSROOM_BRIDGE_MEDIA_HMAC_KEYS_JSON' ) && is_string( NEWSROOM_BRIDGE_MEDIA_HMAC_KEYS_JSON ) ) {
			$parsed = self::parse_key_ring_json( NEWSROOM_BRIDGE_MEDIA_HMAC_KEYS_JSON );
			if ( is_array( $parsed ) ) {
	...
	private static function read_user_id() {
		if ( ! defined( 'NEWSROOM_BRIDGE_MEDIA_USER_ID' ) ) {
			return 0;
		}

		$value = NEWSROOM_BRIDGE_MEDIA_USER_ID;
		if ( is_int( $value ) ) {
	...
	private static function read_max_bytes() {
		if ( ! defined( 'NEWSROOM_BRIDGE_MEDIA_MAX_BYTES' ) || ! is_numeric( NEWSROOM_BRIDGE_MEDIA_MAX_BYTES ) || (int) NEWSROOM_BRIDGE_MEDIA_MAX_BYTES < 1 ) {
			return 500000;
		}

		return (int) NEWSROOM_BRIDGE_MEDIA_MAX_BYTES;
	}
