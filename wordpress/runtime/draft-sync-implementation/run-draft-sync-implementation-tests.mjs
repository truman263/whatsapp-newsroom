...
function productionHashes() {
  return Object.fromEntries([
    ...
    'includes/class-newsroom-bridge-draft-sync-rest.php',
...
function observerOption(name) { return wp(['option', 'get', name], { label: `observer option ${name}`, allowFailure: true }); }
...
async function recreateWordpress() {
  writeRuntimeEnv();
  compose(['up', '-d', '--force-recreate', 'wordpress'], { sensitive: true, label: 'compose config recreate (wordpress)' });
  await waitForHttp();
}

async function recreateWordpressAndCli() {
  writeRuntimeEnv();
  compose(['up', '-d', '--force-recreate', 'wordpress', 'cli'], { sensitive: true, label: 'compose config recreate (wordpress+cli)' });
  await waitForHttp();
}
...
function rawMediaSignedGet({ keyId, secret, mediaKey }) {
  ...
  const canonical = ['newsroom-media-hmac-v1', keyId, 'GET', route, timestamp, mediaKey,
    '-', '-', sha256Buffer(Buffer.alloc(0))].join('\n');
  const signature = createHmac('sha256', decodeDraftSecret(secret)).update(canonical).digest('hex');
...
(async () => {
  try {
    ...
    const { draftStateFingerprint: compiledDraftStateFingerprint, reconciliationFingerprint: compiledReconciliationFingerprint } = requireModule(resolve(runtimeDir, '.build', 'wordpress-draft-state.js'));
    ...
    assert(prodStatus === 'active' && prodVersion === '1.3.0', 'Production plugin activation/version mismatch.');
    assert(wpEval("echo NEWSROOM_BRIDGE_VERSION . '|' . NEWSROOM_BRIDGE_SCHEMA_VERSION . '|' . NEWSROOM_BRIDGE_MEDIA_SCHEMA_VERSION;") === '1.3.0|2|1', 'Version/constants mismatch.');
    ...
    pass('activation_and_schema', { plugin: 'active', bridge: '1.3.0', draft_schema: '2', media_schema: '1', tables: newsroomTables, routes: syncRouteInfo, draft_user: draftUserId, media_user: mediaUserId, production_plugin: 'active' });
    ...
    pass('draft_client_create_201', { handler: 'committed Newsroom_Bridge_REST', loader: 'production plugin', outcome: 'CREATED', post_id: clientCreated.wordpressPostId, author: draftUserId });
    ...
    pass('draft_client_get_200', { handler: 'committed Newsroom_Bridge_REST', loader: 'production plugin', post_id: clientRead.wordpressPostId });
    ...
    pass('draft_client_replay_200', { handler: 'committed Newsroom_Bridge_REST', loader: 'production plugin', outcome: 'REPLAYED', post_id: clientReplay.wordpressPostId });
    ...
    pass('draft_bridge_conflict_409', { handler: 'committed Newsroom_Bridge_REST', loader: 'production plugin', http: 409, preserved_post: clientCreated.wordpressPostId });
    ...
    pass('media_upload_201', { handler: 'committed Newsroom_Bridge_Media_REST', loader: 'production plugin', via: 'media-hmac', attachment: mediaAttachmentA });

    const mediaRead = await rawMediaSignedGet({ keyId: mediaKeyId, secret: mediaSecret, mediaKey: mediaKeyA });
    ...
    assert(mediaReadBody.media_key === mediaKeyA && mediaReadBody.attachment_id === mediaAttachmentA
      && mediaReadBody.status === 'attachment' && mediaReadBody.replayed === true, 'Production media GET DTO disagrees with the committed upload.');
    pass('media_get_200', { handler: 'committed Newsroom_Bridge_Media_REST', loader: 'production plugin', via: 'media-hmac', http: 200, attachment_id: mediaAttachmentA });
    ...
    const parityMatrix = [
      { describe: 'c1_007f', title: 'Parity \u007fTitle', content: 'Parity \u007f body', excerpt: 'Parity \u007f excerpt' },
      { describe: 'c1_009f', title: 'Parity \u0085Title', content: 'Parity \u0085 body', excerpt: 'Parity \u0085 excerpt' },
      { describe: 'ls_2028', title: 'Parity \u2028Title', content: 'Parity \u2028 body', excerpt: 'Parity \u2028 excerpt' },
      { describe: 'ps_2029', title: 'Parity \u2029Title', content: 'Parity \u2029 body', excerpt: 'Parity \u2029 excerpt' },
      { describe: 'mixed_all', title: 'Parity \u007f\u0085\u2028\u2029Title', content: 'Parity \u2028 content \u0085 body \u007f', excerpt: 'Parity excerpt \u2029' },
    ];
    const parityStorageDiffs = [];
    const parityOutcomes = [];
    for (const vector of parityMatrix) {
      const parityTarget = { title: vector.title, content: vector.content, excerpt: vector.excerpt, categories: [categoryIds.catA] };
      ...
      if (vectorPut.status === 200) {
        ...
        const compiledState = compiledDraftStateFingerprint({ title: parityTarget.title, content: parityTarget.content, excerpt: parityTarget.excerpt, categories: parityTarget.categories, featured_media_key: null });
        ...
        const compiledRecon = compiledReconciliationFingerprint({ title: parityTarget.title, content: parityTarget.content, excerpt: parityTarget.excerpt, categories: parityTarget.categories });
        ...
        if (parityPost.title !== parityTarget.title || parityPost.content !== parityTarget.content || parityPost.excerpt !== parityTarget.excerpt) {
          parityStorageDiffs.push({ describe: vector.describe, title: { sent: parityTarget.title, stored: parityPost.title }, content: { sent: parityTarget.content, stored: parityPost.content }, excerpt: { sent: parityTarget.excerpt, stored: parityPost.excerpt } });
        }
        parityOutcomes.push({ describe: vector.describe, outcome: 'applied_parity', fingerprint_match: true });
        ...
        parityOutcomes.push({ describe: vector.describe, outcome: 'wp_storage_rejected_fail_closed', rollback: true });
      } else {
        assert(false, `Unexpected parity outcome (${vector.describe}, status ${vectorPut.status}, text ${vectorPut.text.slice(0, 300)}).`);
      }
    }
    pass('compiled_builder_php_parity', { outcomes: parityOutcomes, byte_exact_storage_diffs: parityStorageDiffs });
    ...
    const dedupeTarget = { title: 'Dedupe Title', content: 'Dedupe body', excerpt: '', categories: [categoryIds.catB, categoryIds.catA, categoryIds.catB] };
    ...
    assert(JSON.stringify(dedupeState.categories) === JSON.stringify(dedupeCategories) && dedupeState.applied_version === stateFingerprint({ title: dedupeTarget.title, content: dedupeTarget.content, excerpt: dedupeTarget.excerpt, categories: dedupeCategories, featured_media_key: null }), 'Deduped state or applied version is inconsistent.');
    pass('category_dedupe_sorted_exact', { deduplicated_twice: 1, sorted: true });
    ...
    assert(featuredSetReconHash === payloadFingerprint({ title: featuredBody.title, content: featuredBody.content, excerpt: featuredBody.excerpt, categories: [categoryIds.catA] }), 'Featured assignment changed the reconciliation fingerprint.');
    ...
    pass('featured_only_payload_hash_invariant', { state_fingerprint_changed: true, reconciliation_fingerprint_stable: true });
    ...
    pass('cas_stale_noop_replay_200', { replayed: true, zero_mutation: true, expected_cas_stale: true });
    ...
    for (let i = 0; i < 10; i += 1) {
      concurrentResults.push(draftPutSync(concurrentKey, concurrentBody.title, concurrentBody.content, concurrentBody.excerpt, concurrentBody.categories, null, concurrentV1).then(
        (value) => ({ status: value.status, replayed: JSON.parse(value.text).replayed }),
        (error) => ({ status: 0, replayed: null, error: String(error?.message ?? error) }),
      ));
    }
    ...
    pass('concurrency_identical_10', { requests: 10, same_expected_version: true, all_200: true, real_transitions: concurrentApplied, replayed: concurrentReplayCount, final_state_exact: true, duplicate_posts: concurrentDuplicates - 1 });
    ...
    const validationBodies = [
      ...
      { describe: 'unsupported_field', body: { ...syncInput(syncKey, 'Title', 'Content', '', [categoryIds.catA], null), post_id: 1, attachment_id: 1, status: 'publish', author: 1 } },
      { describe: 'bad_categories', body: syncInput(syncKey, 'Title', 'Content', '', [], null) },
      { describe: 'unknown_category', body: syncInput(syncKey, 'Title', 'Content', '', [999999999], null) },
    ...
    const arrayReject = await rawDraftSignedRequest('PUT', syncRoute(syncKey), { keyId: draftKeyId, secret: draftSecret, body: Buffer.from('[1,2,3]', 'utf8') });
    ...
    const unknownCategoryReject = await rawDraftSignedRequest('PUT', syncRoute(syncKey), { keyId: draftKeyId, secret: draftSecret, body: syncBody(syncKey, 'Title', 'Content', '', [999999999], null) });
    ...
    assert(afterUnknownCategory.title === priorPostRow.title && afterUnknownCategory.content === priorPostRow.content
      && postThumbnail(syncPostId) === priorPostRow.thumb, 'Unknown category rejection changed durable draft state.');
    pass('unknown_category_rejected_400', { category_id: 999999999, http: 400, code: 'newsroom_category_not_found', durable_state_unchanged: true });
    pass('validation_400s', { scenarios: validationBodies.length + 2, state_preserved: true, array_body: 'newsroom_invalid_payload' });
    ...
    pass('featured_corrupt_file_409', { code: 'newsroom_draft_sync_media_corrupt', repaired: 200 });
    ...
    let uncertainText = '';
    try { const uncertainPut = await draftPutSync(uncertainKey, 'Uncertain Committed', 'Uncertain committed body', '', [categoryIds.catA], null); uncertainStatus = uncertainPut.status; uncertainText = uncertainPut.text; } catch (error) { uncertainStatus = 0; }
    faultPhase(null);
    ...
    pass('fault_commit_uncertain', { code: 'newsroom_reconciliation_outcome_uncertain', durable_commit: true, reconciled_state: uncertainV2, same_replay: 200, next_apply: 200 });
    ...
    const signedPostSync = await rawDraftSignedRequest('POST', syncRoute(syncKey), { keyId: draftKeyId, secret: draftSecret, body: syncBody(syncKey, 'Post Intrusion', 'Post body', '', [categoryIds.catA], null) });
    ...
    const overridePut = await request('PUT', `/wp-json${syncRoute(syncKey)}`, {
      headers: { 'x-newsroom-auth-version': '1', 'x-newsroom-key-id': draftKeyId, 'x-newsroom-timestamp': overrideTimestamp, 'x-newsroom-signature': overrideSignature, 'x-http-method-override': 'PUT', 'content-type': 'application/json' },
    ...
    pass('method_isolation', { signed_post_on_sync_route: 401, x_http_method_override: 401 });
    ...
    pass('cross_route_rejection', { media_to_sync_put: 401, media_to_state: 401, draft_to_media: 401, draft_upload_media: 401 });
    ...
    const routeIsolationProbes = [
      { describe: 'get_on_create_collection', method: 'GET', route: '/newsroom/v1/drafts' },
      { describe: 'get_non_uuid_draft_path', method: 'GET', route: '/newsroom/v1/drafts/not-a-uuid' },
      { describe: 'get_unknown_newsroom_namespace', method: 'GET', route: '/newsroom/v1/other' },
      { describe: 'post_on_state_route', method: 'POST', route: stateRoute(syncKey) },
      { describe: 'put_on_create_collection', method: 'PUT', route: '/newsroom/v1/drafts' },
      { describe: 'patch_on_sync_route', method: 'PATCH', route: syncRoute(syncKey) },
      { describe: 'delete_on_draft_route', method: 'DELETE', route: syncRoute(syncKey) },
      { describe: 'delete_on_state_route', method: 'DELETE', route: stateRoute(syncKey) },
    ];
    ...
    for (const probe of routeIsolationProbes) {
      ...
      const response = await rawDraftSignedRequest(probe.method, probe.route, { keyId: draftKeyId, secret: draftSecret, body });
      routeIsolationResults.push({ describe: probe.describe, status: response.status });
    }
    assert(routeIsolationResults.every((result) => result.status === 401), `Route/method scope was not an exact 401 (${JSON.stringify(routeIsolationResults)}).`);
    pass('auth_route_and_method_exact_401', { probes: routeIsolationResults, unregistered_newsroom_paths: 'fail-closed 401' });
    ...
    const cookieState = await request('GET', `/wp-json${stateRoute(syncKey)}`, {
      headers: { 'x-newsroom-auth-version': '1', 'x-newsroom-key-id': draftKeyId, 'x-newsroom-timestamp': cookieTimestamp, 'x-newsroom-signature': cookieSignature, cookie: `${loggedInCookieName}=deadbeef` },
    });
    ...
    const nonceState = await request('GET', `/wp-json${stateRoute(syncKey)}`, {
      headers: { 'x-newsroom-auth-version': '1', 'x-newsroom-key-id': draftKeyId, 'x-newsroom-timestamp': nonceTimestamp, 'x-newsroom-signature': nonceSignature, 'x-wp-nonce': randomBytes(16).toString('hex') },
    });
    ...
    const batchBody = JSON.stringify({ requests: [{ method: 'PUT', path: syncRoute(syncKey), body: JSON.stringify(syncInput(syncKey, 'Batch Intrusion', 'Batch body', '', [categoryIds.catA], null)), headers: { 'content-type': 'application/json' } }] });
    const signedBatch = await rawDraftSignedRequest('POST', '/batch/v1', { keyId: draftKeyId, secret: draftSecret, body: Buffer.from(batchBody, 'utf8') });
    ...
    const authorsMe = await request('GET', '/wp-json/wp/v2/users/me', { headers: { authorization: `Basic ${authorBasic}` } });
    ...
    const authorNewsroom = await request('PUT', `/wp-json${syncRoute(syncKey)}`, {
      headers: { authorization: `Basic ${authorBasic}`, 'content-type': 'application/json' },
    ...
    pass('auth_ordinary_author_unaffected', { users_me: 200, id: authorUserId, app_passwords: 'available', newsroom_route: 401 });
    ...
    const xmlRpcResponse = await request('POST', '/xmlrpc.php', { headers: { 'content-type': 'text/xml' }, body: xmlRpcBody });
    ...
    pass('auth_generic_login_lockdown', { password_login: 'denied', cookie_signon: 'denied', app_passwords_draft: 'denied', xmlrpc: 'denied' });
    ...
    const databaseAudit = { draft: databaseSecretAudit(draftSecret), media: databaseSecretAudit(mediaSecret) };
    assert(Object.values(databaseAudit).flatMap((counts) => Object.values(counts)).every((count) => count === 0), 'Generated draft/media secret persisted in the runtime database.');
    ...
    try { observerFailStatus = (await draftPutSync(observerFailKey, 'Observer Fail', 'Observer fail body', '', [categoryIds.catA], null)).status; } catch { observerFailStatus = 0; }
    faultPhase(null);
    ...
    pass('auth_service_user_transition', { during_callback: draftUserId, shutdown_after_success: 0, shutdown_after_callback_failure: 0, failure_status: 503 });
    ...
    const configProbe = async () => {
      ...
      return { put_status: put.status, get_status: get.status, put_401: put.status === 401, get_denied: get.status !== 200 };
    };

    runtimeEnv = { ...goodRuntimeEnv };
    runtimeEnv.NEWSROOM_BRIDGE_DRAFT_HMAC_KEYS_JSON = 'not-json';
    await recreateWordpress();
    configFailures.push({ label: 'malformed_key_ring_json', ...(await configProbe()) });

    runtimeEnv = { ...goodRuntimeEnv };
    runtimeEnv.NEWSROOM_BRIDGE_DRAFT_HMAC_KEYS_JSON = '';
    await recreateWordpress();
    configFailures.push({ label: 'absent_key_ring_json', ...(await configProbe()) });

    runtimeEnv = { ...goodRuntimeEnv };
    runtimeEnv.NEWSROOM_BRIDGE_SERVICE_LOCKDOWN_ENABLED = '0';
    await recreateWordpress();
    configFailures.push({ label: 'lockdown_explicitly_disabled', ...(await configProbe()) });
    ...
    configFailures.push({ label: 'hmac_explicitly_disabled', put_status: hmacDisabledPut.status, get_status: hmacDisabledGet.status, put_401: hmacDisabledPut.status === 401, get_denied: hmacDisabledGet.status !== 200 });

    runtimeEnv = { ...goodRuntimeEnv };
    runtimeEnv.NEWSROOM_BRIDGE_USER_ID = '0';
    await recreateWordpress();
    configFailures.push({ label: 'service_user_absent', ...(await configProbe()) });

    assert(configFailures.every((failure) => failure.put_401 && failure.get_denied), `Configuration drift was not fail-closed (${JSON.stringify(configFailures)}).`);
    ...
    pass('auth_config_fail_closed', { variants: configFailures, restored: { http_post: 201, http_put: 200 } });
    ...
    const finalLogs = compose(['logs', '--no-color', 'wordpress'], { sensitive: true }).stdout;
    assert(excludedKnown.every((item) => !finalLogs.includes(item)), 'A generated runtime secret appeared in final WordPress logs.');
    const finalDatabaseAudit = { draft: databaseSecretAudit(draftSecret), media: databaseSecretAudit(mediaSecret) };
    assert(Object.values(finalDatabaseAudit).flatMap((counts) => Object.values(counts)).every((count) => count === 0), 'A generated draft/media secret appeared in final WordPress durable state.');
    ...
    secretEvidence.details.final_scan = { logs: 0, database: finalDatabaseAudit, repository: { draft: 0, media: 0 }, durable_result_checked_during_cleanup: true };
