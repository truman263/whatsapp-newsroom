...
/**
 * TEST-ONLY HMAC authentication for the draft-sync proof surface.
 *
 * This is a line-for-line mirror of the FROZEN production
 * Newsroom_Bridge_Auth with a deliberately small, documented delta:
 * the route allow-list also accepts
 *
 *   - PUT /newsroom/v1/drafts/{draft_key}             draft_sync
 *   - GET /newsroom/v1/drafts/{draft_key}/state       draft_state
 *
 * The canonical string, key-id rules, clock-skew window, temporary service
 * user context, nested-dispatch denial, and finally-restored authority are
 * identical to the frozen production class. Nothing here is production code.
 */
final class Newsroom_Bridge_Draft_Sync_Auth {
	...
	public function authenticate( $result ) {
		...
		if ( in_array( $method, array( 'POST', 'PUT' ), true ) && ! $this->json_content_type_is_valid() ) {
	...
	private function matches_proof( $request, $routed = true ) {
		...
		$p = $this->proof;
		$expected_url = $this->expected_url_params( $p['route'], $p['method'] );
	...
	private function expected_url_params( $route, $method ) {
		if ( 'POST' === $method ) {
			return array();
		}
		$prefix = strlen( '/newsroom/v1/drafts/' );
		if ( 'GET' === $method && 0 === substr_compare( $route, '/state', -strlen( '/state' ) ) ) {
			return array( 'draft_key' => substr( $route, $prefix, -strlen( '/state' ) ) );
		}
		return array( 'draft_key' => substr( $route, $prefix ) );
	}
	...
	/** Wrap only the frozen bridge handlers plus the two proof sync handlers, never a namespace. */
