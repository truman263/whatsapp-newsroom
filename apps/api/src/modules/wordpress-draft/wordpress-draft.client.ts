...
import { normalizeWordPressBaseUrl, SecureWordPressTransport, WordPressTransportSecurityError, type WordPressTransportResponse } from "../wordpress-transport/wordpress-secure-transport";

export type TransportResponse = WordPressTransportResponse;
...
export class WordPressDraftClient {
  ...
  constructor(
    private readonly options: WordPressDraftClientOptions,
  ...
  async syncDraft(input: SyncWordPressDraftInput): Promise<SyncWordPressDraftResult> {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        try {
          current = await this.getDraftState(payload.draft_key);
        } catch {
          throw new WordPressDraftError("UNCERTAIN_OUTCOME", "WordPress sync could not be reconciled.");
        }
        if (current.applied_version === desired)
          return {
            draft_key: current.draft_key,
            post_id: current.post_id,
            status: "draft",
            featured_media_key: current.featured_media_key,
  ...
  private async postOrReconcile(draftKey: string, rawBody: string, mayRetryPost: boolean): Promise<CreateWordPressDraftResult> {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        return {
          wordpressDraftKey: value.wordpressDraftKey,
          wordpressPostId: value.wordpressPostId,
          status: value.status,
          outcome: value.replayed ? "REPLAYED" : "CREATED",
        };
      }
      throw classifyStatus(response.status);
    } catch (error) {
  ...
  private async send(method: "GET" | "POST" | "PUT", route: string, rawBody = ""): Promise<TransportResponse> {
    ...
    try {
      ...
      if (error instanceof WordPressTransportSecurityError) throw new WordPressDraftError("CONTRACT_FAILURE", "WordPress transport security policy rejected the request.");
      throw new WordPressDraftError("UNCERTAIN_OUTCOME", "WordPress request outcome is uncertain.");
    }
  }
...
const normalizeBaseUrl = normalizeWordPressBaseUrl;

function validateDraftKey(value: string): void {
  if (!UUID_V4.test(value)) throw new WordPressDraftError("CONTRACT_FAILURE", "Invalid WordPress draft key.");
}
...
function validateBridgeResponse(body: unknown, creation: boolean): WordPressDraftReference & { replayed?: boolean } {
  ...
  return {
    wordpressDraftKey: value.draft_key,
    wordpressPostId: value.post_id as number,
    status: "draft",
    ...(creation ? { replayed: value.replayed as boolean } : {}),
  };
}
