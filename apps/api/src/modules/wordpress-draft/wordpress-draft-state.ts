...
export function canonicalStateJson(state: CanonicalDraftState): string {
  ...
  return JSON.stringify({
    ...
    categories: [...new Set(state.categories)].sort((a, b) => a - b),
...
export function syncPayload(input: SyncWordPressDraftInput): CanonicalDraftState & {
  draft_key: string;
  expected_version: string | null;
} {
  const allowed = ["wordpressDraftKey", "headline", "body", "excerpt", "wordpressCategoryIds", "editorialByline", "featuredMediaKey", "expectedVersion"];
  if (!input || Object.keys(input).some((k) => !allowed.includes(k)) || !isDraftKey(input.wordpressDraftKey) || !validString(input.headline) || !input.headline.trim() || !validString(input.body) || !input.body.trim() || (input.excerpt !== undefined && !validString(input.excerpt)) || !Array.isArray(input.wordpressCategoryIds) || !input.wordpressCategoryIds.length || !input.wordpressCategoryIds.every(positiveId) || !validString(input.editorialByline) || !input.editorialByline.trim() || Array.from(input.editorialByline).length > 200 || !mediaKey(input.featuredMediaKey) || (input.expectedVersion !== undefined && input.expectedVersion !== null && !isStateVersion(input.expectedVersion))) throw new WordPressDraftError("CONTRACT_FAILURE", "Invalid WordPress sync input.");
  return {
    draft_key: input.wordpressDraftKey,
    title: input.headline,
    content: input.body,
    excerpt: input.excerpt ?? "",
    categories: [...new Set(input.wordpressCategoryIds)].sort((a, b) => a - b),
...
export function validateStateResponse(body: unknown, key: string, state: boolean): WordPressDraftState | Omit<SyncWordPressDraftResult, "outcome"> {
  const bad = (): never => {
    throw new WordPressDraftError("UNEXPECTED_RESPONSE", "WordPress returned an invalid draft state.");
  };
  if (!body || typeof body !== "object" || Array.isArray(body)) return bad();
  const b = body as Record<string, unknown>;
  const fields = ["draft_key", "post_id", "status", "featured_media_key", "applied_version", ...(state ? ["title", "content", "excerpt", "categories", "editorial_byline", "author_id"] : ["replayed"])];
  ...
  if (state) {
    if (!validString(b.title) || !validString(b.content) || !validString(b.excerpt) || !validString(b.editorial_byline) || Array.from(b.editorial_byline).length > 200 || !positiveId(b.author_id) || !Array.isArray(b.categories) || !b.categories.length || !b.categories.every(positiveId) || b.categories.some((id: number, i: number) => i > 0 && id <= ((b.categories as number[])[i - 1] ?? 0))) return bad();
