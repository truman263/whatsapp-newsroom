...
export type MediaTransportResponse = WordPressTransportResponse;
...
export class WordPressMediaClient {
  ...
  constructor(
    private readonly options: WordPressMediaClientOptions,
    ...
    if (!Number.isInteger(options.requestTimeoutMs) || options.requestTimeoutMs < 100 || !Number.isInteger(options.reconciliationAttempts) || options.reconciliationAttempts < 1 || !Number.isInteger(options.reconciliationDelayMs) || options.reconciliationDelayMs < 0 || !Number.isInteger(options.maxBytes) || options.maxBytes < 1) throw new Error("Invalid WordPress media client configuration.");
  ...
  private async send(method: "GET" | "POST", route: string, mediaKey: string, filename: string | null, mimeType: string | null, body: Buffer): Promise<MediaTransportResponse> {
    ...
    try {
      ...
      if (error instanceof WordPressTransportSecurityError) throw new WordPressMediaError("CONTRACT_FAILURE", "WordPress transport security policy rejected the request.");
...
function validateMediaKey(value: string): void {
  if (!UUID_V4.test(value)) throw new WordPressMediaError("CONTRACT_FAILURE", "Invalid media key.");
}
...
function validateMediaResponse(body: unknown):
  | {
      mediaKey: string;
      attachmentId: null;
      status: "reserved";
  ...
  return {
    mediaKey: value.media_key,
    attachmentId: value.attachment_id as number,
    status: "attachment",
    replayed: value.replayed,
  };
}
