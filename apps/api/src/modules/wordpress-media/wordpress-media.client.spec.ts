...
const createClient = (transport: WordPressMediaTransport, overrides: Partial<{ reconciliationAttempts: number; reconciliationDelayMs: number; baseUrl: string }> = {}, now: () => number = () => FIXED_NOW): WordPressMediaClient => new WordPressMediaClient({
...
class MockTransport implements WordPressMediaTransport {
  public requests: Array<{ method: string; url: string; headers: Readonly<Record<string, string>>; body?: Buffer }> = [];
  ...
  async send(request: { method: 'GET' | 'POST'; url: string; headers: Readonly<Record<string, string>>; body?: Buffer; timeoutMs: number }): Promise<MediaTransportResponse> {
    ...
    this.requests.push({ method: request.method, url: request.url, headers: request.headers, body: request.body });
...
const created = (extra: Partial<Record<string, unknown>> = {}): MediaTransportResponse => ({ status: 201, body: { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment', replayed: false, ...extra } });
const replayed = (extra: Partial<Record<string, unknown>> = {}): MediaTransportResponse => ({ status: 200, body: { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment', replayed: true, ...extra } });
const attachment = (extra: Partial<Record<string, unknown>> = {}): MediaTransportResponse => ({ status: 200, body: { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment', replayed: true, ...extra } });
...
describe('WordPressMediaClient', () => {
  it('POST: transmits the exact binary Buffer as the request body with application/octet-stream', async () => {
    ...
    const result = await client.uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
    expect(result).toEqual({ mediaKey: MEDIA_KEY, attachmentId: 41, status: 'attachment', outcome: 'CREATED' });
  ...
  it('POST: never sends Authorization, Cookie or X-WP-Nonce headers', async () => {
    const transport = new MockTransport([created()]);
    const client = createClient(transport);
    await client.uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
    const post = requestAt(transport, 0);
  ...
  it('POST: replayed 200 returns a REPLAYED outcome', async () => {
    ...
    const result = await createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
  ...
  it('POST: a body over the byte limit is rejected without a transport call', async () => {
    ...
    await expect(createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: toobits })).rejects.toMatchObject({ code: 'PAYLOAD_REJECTED' });
    expect(transport.requests).toHaveLength(0);
  });

  it('POST: maps status codes to the strict media error taxonomy', async () => {
    const cases: Array<[number, string, number | undefined]> = [[400, 'CONTRACT_FAILURE', 400], [401, 'AUTHENTICATION_FAILURE', 401], [403, 'AUTHENTICATION_FAILURE', 403], [404, 'NOT_FOUND', 404], [409, 'CONFLICT', 409], [413, 'PAYLOAD_REJECTED', 413], [503, 'IN_PROGRESS', 503], [418, 'UNEXPECTED_RESPONSE', 418]];
    for (const [status, code, httpStatus] of cases) {
      const transport = new MockTransport([{ status, body: {} }]);
      await expect(createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY })).rejects.toMatchObject({ code, ...(httpStatus === undefined ? {} : { httpStatus }) });
    }
  });
  ...
  it('GET: sends no body and no Content-Type/Content-Length headers', async () => {
    ...
    expect(result).toEqual({ mediaKey: MEDIA_KEY, attachmentId: 41, status: 'attachment' });
  ...
  it('GET: a reserved row surfaces as IN_PROGRESS', async () => {
    const transport = new MockTransport([{ status: 200, body: { media_key: MEDIA_KEY, attachment_id: null, status: 'reserved', replayed: false } }]);
  ...
  it('response validation: exactly the strict creation fields, types and values are accepted', async () => {
    const malformed: unknown[] = [
      { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment', replayed: false, extra: 1 },
      { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment' },
      { media_key: MEDIA_KEY, attachment_id: 41, status: 'attachment', replayed: 'yes' },
      { media_key: MEDIA_KEY, attachment_id: 0, status: 'attachment', replayed: false },
      { media_key: MEDIA_KEY, attachment_id: '41', status: 'attachment', replayed: false },
      { media_key: 'not-a-key', attachment_id: 41, status: 'attachment', replayed: false },
      { media_key: MEDIA_KEY, attachment_id: 41, status: 'video', replayed: false },
      [],
      null,
      'oops',
    ];
    for (const body of malformed) {
      ...
      await expect(createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY })).rejects.toMatchObject({ code: 'UNEXPECTED_RESPONSE' });
    }
  });
  ...
  it('reconciliation: an uncertain POST recovers success via a signed GET on the same key', async () => {
    ...
    const result = await createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
    expect(result).toEqual({ mediaKey: MEDIA_KEY, attachmentId: 41, status: 'attachment', outcome: 'RECOVERED' });
    expect(transport.requests.map((request) => request.method)).toEqual(['POST', 'GET']);
  ...
  it('reconciliation: a dropped POST followed by a definite 404 retries the POST exactly once with a fresh timestamp and identical payload', async () => {
    ...
    const result = await createClient(transport, {}, steppedNow).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
  ...
  it('reconciliation: an uncertain POST followed by an uncertain GET never triggers a blind second POST', async () => {
    ...
    await expect(createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY })).rejects.toMatchObject({ code: 'UNCERTAIN_OUTCOME' });
  ...
  it('reconciliation: a GET failure that is a definite IN_PROGRESS is not retried', async () => {
    ...
    await expect(createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY })).rejects.toMatchObject({ code: 'IN_PROGRESS' });
    expect(transport.requests).toHaveLength(2);
  });

  it('reconciliation: a GET-backed reserved row is POSTed to only once more', async () => {
    const transport = new MockTransport(['throw-unavailable', { status: 200, body: { media_key: MEDIA_KEY, attachment_id: null, status: 'reserved', replayed: false } }, replayed()]);
    const result = await createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY });
    expect(result.outcome).toBe('REPLAYED');
    expect(transport.requests.map((request) => request.method)).toEqual(['POST', 'GET', 'POST']);
  });
  ...
  it('errors: never include the secret or credential material in messages', async () => {
    ...
    const error = await createClient(transport).uploadMedia({ mediaKey: MEDIA_KEY, filename: FILENAME, mimeType: MIME, body: KAT_BODY }).catch((caught: unknown) => caught);
  ...
  it('configuration: invalid base URL, key id or numeric bounds are rejected at construction', () => {
    ...
    expect(() => new WordPressMediaClient({ baseUrl: BASE_URL, keyId: 'Not Valid!', secret: SECRET, requestTimeoutMs: REQUEST_TIMEOUT_MS, reconciliationAttempts: 3, reconciliationDelayMs: 0, maxBytes: MAX_BYTES }, transport, () => FIXED_NOW)).toThrow();
    expect(() => new WordPressMediaClient({ baseUrl: BASE_URL, keyId: KEY_ID, secret: 'not-a-secret', requestTimeoutMs: 0, reconciliationAttempts: 3, reconciliationDelayMs: 0, maxBytes: MAX_BYTES }, transport, () => FIXED_NOW)).toThrow();
  });
